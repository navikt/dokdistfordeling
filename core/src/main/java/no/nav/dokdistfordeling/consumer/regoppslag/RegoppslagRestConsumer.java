...
class RegoppslagRestConsumer {
    ...
    HentMottakerOgAdresseResponseTo.AdresseTo hentAdresse(HentMottakerOgAdresseRequestTo request) {
        ...
        try {
            ...
            throw new RegoppslagHentAdresseTechnicalException(String.format("Kall mot TREG002 feilet teknisk. status=%s, feilmelding=%s", e
                    .getStatusCode(), e.getMessage()), e);
        }
    }
    ...
    private HttpHeaders retrieveSamlTokenAndCreateHeader() {
        try {
            ...
            throw new RegoppslagHentAdresseTechnicalException(String.format("Henting av samltoken fra STS feilet. Feilmelding=%s", e.getMessage()), e);
        }
    }

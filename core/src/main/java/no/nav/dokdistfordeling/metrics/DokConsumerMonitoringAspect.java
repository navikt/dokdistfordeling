...
public class DokConsumerMonitoringAspect {
	private final MeterRegistry registry;
	private final Function<ProceedingJoinPoint, Iterable<Tag>> tagsBasedOnJoinpoint;

	public DokConsumerMonitoringAspect(MeterRegistry registry) {
		this(registry, pjp ->
	...
	private DokConsumerMonitoringAspect(MeterRegistry registry, Function<ProceedingJoinPoint, Iterable<Tag>> tagsBasedOnJoinpoint) {
		this.registry = registry;
		this.tagsBasedOnJoinpoint = tagsBasedOnJoinpoint;
	}

	@Around("execution (@no.nav.dokdistfordeling.metrics.ConsumerMonitor * *.*(..))")
	public Object dokMetrics(ProceedingJoinPoint pjp) throws Throwable {
		Method method = ((MethodSignature) pjp.getSignature()).getMethod();
		ConsumerMonitor monitor = method.getAnnotation(ConsumerMonitor.class);
